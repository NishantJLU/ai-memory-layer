...
@mcp.tool()
async def recall_memory(query: str, project_id: str = "default", limit: int = 5) -> str:
    """
    ...
    async with AsyncSessionLocal() as db:
        memories = await recall_memories(query, project_id, limit, db=db)
...
@mcp.tool()
async def store_memory(content: str, memory_type: str, module: str, project_id: str = "default") -> str:
    """
...
@mcp.tool()
async def list_recent_memories(project_id: str = "default", module: str = None, limit: int = 10) -> str:
    """
    Surface what is known about a specific module sorted by recency.
    """
    async with AsyncSessionLocal() as db:
        try:
            stmt = select(Memory).filter(Memory.project_id == project_id)
            if module:
                stmt = stmt.filter(Memory.module == module)

            stmt = stmt.order_by(Memory.date.desc()).limit(limit)
...
@mcp.tool()
async def flag_contradiction(memory_id: int, reason: str) -> str:
    """
